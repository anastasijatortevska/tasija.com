...
Counting them is the classic warm-up: for each vertex $$i$$, look at pairs of its neighbors and check whether they're adjacent. With adjacency sets that's $$O\!\left(\sum_i d_i^2\right)$$, which is fine at this size. The linear-algebra version is prettier: if $$A$$ is the adjacency matrix, then
